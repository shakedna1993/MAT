...
public class StatisticReportCCGuIController implements Initializable {
	...
	public void setDiagramInfo(){
		...
		for(int i=0; i<courselist.size();i++)
			if((Integer.parseInt(courselist.get(i).getSemid())<sts)||(Integer.parseInt(courselist.get(i).getSemid())>ens))
				courselist.remove(i);
		
		
		for(int i=0; i<courselist.size();i++){
			MyThread b = new MyThread(RequestType.CourseGradeList, IndexList.CourseGradeList,courselist.get(i).getCourseId());
			b.start();
			try {
				b.join();
				} catch (InterruptedException e1) {
		...
		for(int i=0; i<courselist.size();i++){
			...
			MyThread ct = new MyThread(RequestType.createCourseEntity, IndexList.createCourseEntity,courselist.get(i).getCourseId());
			ct.start();
			try {
				ct.join();
				} catch (InterruptedException e1) {
